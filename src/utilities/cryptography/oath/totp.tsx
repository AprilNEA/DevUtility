...
import { Copy, Clock, Shield, Key, CheckCircle, XCircle, QrCode, Smartphone } from "lucide-react";
import { useUtilityInvoke } from "@/utilities/invoke";
import { InvokeFunction, TotpHashAlgorithm, TotpSecret, TotpResult, TotpValidationResult } from "@/utilities/types";
...
export default function TotpDebugger() {
  ...
  const [algorithm, setAlgorithm] = useState<TotpHashAlgorithm>(TotpHashAlgorithm.SHA1);
  ...
  const [validationResult, setValidationResult] = useState<TotpValidationResult | null>(null);

  // Hooks for backend calls
  const { trigger: generateSecret, isMutating: generatingSecret } = useUtilityInvoke(InvokeFunction.GenerateTotpSecret);
  const { trigger: generateCode, isMutating: generatingCode } = useUtilityInvoke(InvokeFunction.GenerateTotpCode);
  const { trigger: validateCode, isMutating: validatingCode } = useUtilityInvoke(InvokeFunction.ValidateTotpCode);
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
        ...
        <div className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="px-6 py-4">
            <h1 className="text-2xl font-semibold tracking-tight mb-4">TOTP Debugger</h1>
            <TabsList className="grid w-full grid-cols-3 bg-muted/50">
              ...
              <TabsTrigger value="validate" className="data-[state=active]:bg-background">
                <div className="flex items-center gap-2">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="bg-muted/50 rounded-lg p-4">
                    <p className="text-sm font-medium mb-2">
                      💡 The most common parameters used by services is a <strong>period of 30 seconds</strong>, <strong>sha1 algorithm</strong> and <strong>6 Digits</strong> for your OTP code. If you are not sure what parameters are used by your service, always use this as default.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={algorithm} onValueChange={(value) => setAlgorithm(value as TotpHashAlgorithm)}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <Input value={totpSecret.secret} readOnly className="font-mono text-sm" />
                        <Button
                          size="icon"
                          variant="outline"
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <Input value={totpSecret.provisioningUri} readOnly className="font-mono text-xs" />
                        <Button
                          size="icon"
                          variant="outline"
                          onClick={() => copyToClipboard(totpSecret.provisioningUri)}
                        >
                          <Copy className="w-4 h-4" />
                        </Button>
                      </div>
          ...
          <TabsContent value="generate" className="space-y-6 mt-0">
            <div className="grid lg:grid-cols-2 gap-6 h-full">
              ...
              <Card className="h-fit">
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="secret-generate">Enter a generated TOTP (Time based) secret</Label>
                    <Input
                      id="secret-generate"
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        max={300}
                      />
                      <p className="text-xs text-muted-foreground">What period used for the secret</p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">What algorithm was used for the secret</p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        max={8}
                      />
                      <p className="text-xs text-muted-foreground">How much digits set for your created secrete</p>
                    </div>
                  </div>

                  <Button onClick={handleGenerateCode} className="w-full" disabled={!secret || generatingCode}>
              ...
              <Card className="h-fit">
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      Normally the secret is securely saved on server-side DB, however on test you have to enter manually
                    </p>
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="code-validate">Generated TOTP Code to auth</Label>
                    <Input
                      id="code-validate"
                      placeholder="Code ..."
                    ...
                    />
                    <p className="text-xs text-muted-foreground">Based on your secret</p>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">What algorithm used for the secret</p>
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      Number of time windows to check before and after current time (0 = only current window)
                    </p>
                  </div>
                    ...
                    <div className="space-y-3 text-sm">
                      ...
                      {validationResult.timeOffset !== 0 && (
                        <div className="flex justify-between">
                          ...
                          <span className={validationResult.timeOffset > 0 ? "text-blue-600" : "text-orange-600"}>
                            {validationResult.timeOffset > 0 ? "+" : ""}{validationResult.timeOffset} windows
                          </span>
                        </div>
                      )}
                      <div className="flex justify-between">
                      ...
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Used Window:</span>
